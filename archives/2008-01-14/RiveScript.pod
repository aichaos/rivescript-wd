...
RiveScript 2.00 Working Draft (2008/01/14)
...
Example:
  ...
  > object encode perl
    ...
    elsif ($method eq 'base64') {
      return encode_base64 ($args);
    }
...
For example:
  ...
  + encode * in md5
  - The MD5 hash of "<star>" is: &encode.md5(<star>)

  + encode * in base64
  - The Base64 hash of "<star>" is: &encode.base64(<star>)
...
Assign a weight to a random response. This tag can only be used with
C<- Response>.
